defmodule Structs do
  ...
  koan "Use the put_in macro to replace a nested value" do
    airline = %Airline{}
    assert put_in(airline.plane.maker, :airbus) == %Structs.Airline{plane: %Structs.Plane{maker: :airbus}, name: "Southwest"}
  end

  koan "Use the update_in macro to modify a nested value" do
    airline = %Airline{plane: %Plane{passengers: 200}}
    assert update_in(airline.plane.passengers, fn x -> x + 2 end) == %Structs.Airline{plane: %Structs.Plane{passengers: 202}, name: "Southwest"}
  end
